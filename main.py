...
import framebuf
pin_blue = Pin(14, Pin.IN)
pin_red = Pin(15, Pin.IN)
...
def draw_bg():
    ...
    oled.text("{0} km makerfabs".format(status["km"]),2,0)
    if(bg["x"]<=-10):
        bg["x"]=0
...
while (True):
    ...
    if(status["game"]=="loading"):
        ...
        oled.text("-> 1 Player".format(status["km"]),15,10)
        oled.text("Start    Jump".format(status["km"]),10,30)
        oled.text("-Makerfabs.com-".format(status["km"]),2,55)
    elif(status["game"]=="pause"):
        ...
        oled.text("makerfabs",25,20)
        oled.blit(gameover_buf, 0,28)
    oled.show()
